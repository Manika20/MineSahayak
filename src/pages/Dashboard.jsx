...
const Dashboard = () => {
  ...
  const MyOffcanvas = () => {
    return (
      <Offcanvas className="fixed top-0 md:hidden w-1/3 bottom-0 left-0 border bg-gray-800 text-white p-4" show={showOffcanvas} onHide={handleOffcanvas}>
  ...
  <Button className="border-0 mt-4 bg-transparent text-white" onClick={handleOffcanvas}>
   ...
   <MyOffcanvas />
    ...
    <div className="flex justify-between w-full">
      <div></div>
      <div className=" text-3xl md:text-5xl  h-auto md:h-14 mt-5 tracking-tighter text-transparent  bg-clip-text bg-gradient-to-r from-blue-400 via-blue-600 to-blue-800 font-bold">Minesahayak <span className="hidden lg:inline">Chatbot</span></div>
      <div className="w-[30px] md:w-[50px] bg-yellow-200 rounded-full text-center flex items-center justify-center mr-5  mt-6">U</div>
      </div>
